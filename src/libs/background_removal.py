...
def remove_background_kmeans(myimage):
    # Read the image
    myimage = cv2.imread(myimage)
    myimage = cv2.cvtColor(myimage, cv2.COLOR_BGR2RGB)  # Convert to RGB for displaying in matplotlib
    ...
    if np.mean(centers[0]) > np.mean(centers[1]):
        background[mask == 0] = [255, 255, 255]  # Set background to white
        foreground[mask == 1] = myimage[mask == 1]
    else:
        background[mask == 1] = [255, 255, 255]  # Set background to white
